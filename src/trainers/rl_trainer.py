...
class RLTrainer():
    ...
    def __init__(self,
        ...
        self.set_optimizer()
        # self.date_str = "_" + date.today().strftime("%m%d")
        self.date_str = ""
        self.output_path = self.args.output_dir + self.policy_model.model_name.split("/")[-1] + self.date_str + self.args.output_suffix + "_step_0"
    ...
    def log_grad_norm(self):
        ...
        self.logger.info(f"Total grad norm: {total_norm}")
    ...
    def _format_data(self, data):
        ...
        dataset_prefixes = [DATASET_TO_PREF.get(d, "") for d in data['dataset']] if instruction != "" else ["" for d in data['dataset']]
    ...
    def generate_data(self, step: int = 0, batch: Dict[str, str] = None, previous_batch: pd.DataFrame = None):
        ### Rollouts from policy
        columns = ["_id", "question", "answer", "thought", "response", "dataset"]
        try:
            new_path = self.args.data_dir + f"/train_{self.policy_model.model_name}{self.date_str}_{self.type}_step_{step}.json"
        ...
        use_hint = True
        ### Add previous batch to current batch
        if previous_batch is not None and (self.type == "star"):
            for k, v in batch.items():
                batch[k] = v + previous_batch.get(k, [])
        ### Iterate over batch to generate data
        batch = self.sample_rollouts(batch, hint=use_hint, step=step).to_dict()
        new_dataset = Dataset.from_dict(batch)
        ### Save dataset
        new_path = self.args.data_dir + f"/train_{self.policy_model.model_name}{self.date_str}_{self.type}_step_{step}.json"
    ...
    def train(self):
        ...
        for step, batch in enumerate(tqdm(self.train_dataloader)):
            ...
            if self.args.start_step > step:
                previous_batch = {k: v + previous_batch[k] for k, v in batch.items()} if previous_batch else batch
                continue
            self.policy_model.eval()
            if step == self.args.start_step:
                self.load_policy_model(step)
            new_data = self.generate_data(step, batch, previous_batch)
            previous_batch = {k: v + previous_batch[k] for k, v in batch.items()} if previous_batch else batch
            batch = new_data
            ### Reset model
            if self.type == "star":
